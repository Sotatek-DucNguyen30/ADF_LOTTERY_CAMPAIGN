...
║  Thay <TARGET_SCREEN_ID> (VD SLC0104) và <SHEET_NAME> (VD "Screen_SLC0104      ║
...
═══════════════════════════════════════════════════════════════════════════════
GIAI ĐOẠN I — DỰNG / HOÀN THIỆN SHEET CHO <TARGET_SCREEN_ID>
═══════════════════════════════════════════════════════════════════════════════

Nhiệm vụ: mở file output đã có "Screen List" + "Screen Transition Diagram", đảm bảo tồn tại
1 sheet RIÊNG cho ĐÚNG 1 màn hình có Screen ID = <TARGET_SCREEN_ID> (mỗi SLC ID là 1 sheet
riêng, KHÔNG gộp 5-6 tab cùng nhóm vào 1 sheet). Nếu sheet chưa có → tạo mới; nếu đã có →
hoàn thiện theo cấu trúc chuẩn dưới đây (KHÔNG tạo sheet trùng).

──────────────────────────────────────────────
I.-1 — Xác định nguồn sub-screen cho <TARGET_SCREEN_ID>
──────────────────────────────────────────────
NGUỒN CHÍNH: sheet "Final_Screen List" trong file
  inputs/screen-detail/screen-list/Screen_List_Ver1.2.xlsx
...
xuống dòng), H=Sub function/ sub layout (Figma mapping) (các sub-screen/state con — mỗi dòng
1 mục, có hậu tố phân loại "(sub layout)" hoặc "(sub function)" ở cuối chuỗi).

⚠ PHÂN BIỆT 2 LOẠI MỤC TRONG CỘT H (đọc hậu tố cuối chuỗi của từng dòng):
  • "(sub function)" → là 1 MÀN HÌNH RIÊNG BIỆT. Tách ra thành 1 SUB-SCREEN CON của Screen ID
    đó, dựng ĐẦY ĐỦ 4 mục như mọi màn hình (Overview / Layout+ảnh / Input-Output / Actions).
  • "(sub layout)" → KHÔNG phải màn hình riêng. Nó NẰM CHUNG layout với màn hình đang thực
    hiện, chỉ để mô tả THÊM về required / thao tác / hành vi ở layout đó. Chỉ cần gộp mô tả
    (và ảnh nếu có) vào mục (2) Screen Layout của màn hình cha — KHÔNG tách thành sub-screen
    riêng, KHÔNG tạo block 4 mục riêng cho nó.
  Dòng H không có hậu tố rõ ràng → mặc định coi như "(sub function)" (tách riêng) và ghi chú
  "⚠ H thiếu nhãn phân loại — cần xác nhận".

CÁCH XÁC ĐỊNH SUB-SCREEN cho <TARGET_SCREEN_ID> (đọc động từ file, KHÔNG hardcode):
1. Tìm dòng có cột B = <TARGET_SCREEN_ID>. Lấy Title (C), URL (E), breadcrumb (D), 備考 (F).
2. State/sub-screen = hợp của:
   • Các dòng trong cột G (Figma mapping) của chính dòng đó — mỗi dòng là 1 state.
   • Các dòng trong cột H (Sub function/ sub layout) của dòng đó VÀ các dòng NGAY DƯỚI có cột B
     trống (None) cho tới trước Screen ID kế tiếp. Đọc HẬU TỐ phân loại của từng dòng H:
       – Dòng có "(sub function)" → 1 SUB-SCREEN riêng (đưa vào danh sách sub-screen, đủ 4 mục).
       – Dòng có "(sub layout)" → KHÔNG phải sub-screen riêng; gom vào phần mô tả layout của
         màn hình cha (mục 2). KHÔNG cấp Screen ID con, KHÔNG đưa vào danh sách sub-screen.
...
4. Gán Screen ID hiển thị cho từng sub-screen dạng "<TARGET_SCREEN_ID>.<số thứ tự>" theo đúng
   thứ tự xuất hiện trong G rồi H — CHỈ cho các state (G) và các dòng H "(sub function)". Các
   dòng H "(sub layout)" KHÔNG được cấp số thứ tự vì không phải màn hình riêng
   (VD 3 state của SLC0104 → SLC0104.1/.2/.3).

BẢNG THAM CHIẾU NHANH (chốt từ Final_Screen List — VẪN đối chiếu lại file khi chạy, ưu tiên
dữ liệu file nếu có sai lệch):

  | <TARGET_SCREEN_ID> | Title / màn hình | State & sub-screen (theo cột G / H) |
  |---|---|---|
...
⚠ Với các mục "sub: ..." trong bảng (lấy từ cột H): khi chạy PHẢI đọc hậu tố "(sub function)" /
"(sub layout)" của từng dòng H để quyết định tách sub-screen riêng (sub function) hay chỉ gộp
vào Layout của màn hình cha (sub layout) — xem quy tắc phân loại ở I.-1 và NC6.
...
- `ui-ux-pro-max`  — phân tích ảnh trong inputs/screen-detail/image-screen/ khớp với
  <TARGET_SCREEN_ID> và sub-screen của nó (theo mapping I.-1); trích component/state/hành vi,
  xác định path ảnh để CHÈN ẢNH THẬT.
- `design-system`  — chuẩn hoá "Type" component theo taxonomy sheet "Settings" của file
  inputs/translated_【基本設計書】....xlsx; PHẢI nhất quán với các sheet Screen N đã tạo
  trước đó (đọc lại output hiện có).
...
──────────────────────────────────────────────
I.2 — Dựng nội dung, tiêu đề đầu sheet = "<TARGET_SCREEN_ID> <Screen Name>"
──────────────────────────────────────────────
(Ở GIAI ĐOẠN I nội dung viết bằng tiếng Nhật/gốc — dịch để dành GIAI ĐOẠN III.)
  ...
  5. Sub Screens (画面内訳) — theo cây map ở I.-1, mỗi sub-screen 1 block (đủ 4 mục trên,
     ...
     ⚠ CHỈ tạo block sub-screen cho các state (cột G) và các dòng cột H có nhãn "(sub function)".
     Dòng cột H có nhãn "(sub layout)" KHÔNG tạo block riêng — thay vào đó gộp mô tả của nó
     (required/thao tác/hành vi + ảnh nếu có) vào mục (2) Screen Layout của màn hình cha (hoặc
     của đúng state mà nó thuộc về).

  ⚠ Lưu ý cấu trúc main vs sub-screen: xem NC6 ở GIAI ĐOẠN II — nếu <TARGET_SCREEN_ID> có
...
═══════════════════════════════════════════════════════════════════════════════
GIAI ĐOẠN II — ĐỐI CHIẾU & BỔ SUNG NỘI DUNG (NC1–NC9) cho <TARGET_SCREEN_ID>
═══════════════════════════════════════════════════════════════════════════════

Nhiệm vụ: ĐỐI CHIẾU và BỔ SUNG (KHÔNG tạo sheet mới) cho sheet <SHEET_NAME> — nguồn chính là
...
⚠ PHÂN LOẠI MỤC CỘT H (Screen_List_Ver1.2.xlsx) trước khi dựng cấu trúc:
  • "(sub function)" = màn hình riêng → là 1 SUB-SCREEN đầy đủ 4 mục (xử lý như state bên dưới).
  • "(sub layout)" = KHÔNG phải màn hình riêng → chỉ mô tả thêm required/thao tác/hành vi,
    NẰM CHUNG mục (2) Screen Layout của màn hình/state cha. KHÔNG tách block sub-screen riêng,
    KHÔNG cấp Screen ID con, KHÔNG dựng 4 mục riêng cho nó. Field/action mà "(sub layout)" bổ
    sung được viết vào bảng (3)/(4) của chính màn hình/state cha mà nó thuộc về.

Nếu Screen ID có nhiều state (VD 6 state của SLC0104, 2 state 作成前/作成後 của SLC0101...) hoặc
có dòng H "(sub function)":
...
▸ MỖI SUB-SCREEN (state hoặc dòng H "(sub function)") = 1 MÀN HÌNH RIÊNG ĐẦY ĐỦ 4 mục:
...
───────────────────────────────────────────────────────────────
1. Output đang sửa:
   /home/sotatek/DucNguyen/ADF_Lottery_Campaign/output/[Lottery Campaign ] Screen Definition_07202026_Ver1.2.xlsx
...
3. Function List (danh sách tính năng — ĐÃ dịch sẵn tiếng Việt đầy đủ):
   /home/sotatek/DucNguyen/ADF_Lottery_Campaign/inputs/requirement/business-requirement/Function_List_v1.2.xlsx
...
4. Comment Figma:
   /home/sotatek/DucNguyen/ADF_Lottery_Campaign/inputs/screen-detail/spec-note/List_Comment_Figma_Ver1.2.xlsx
   Sheet: "Comment Figma"
...
import os
files_to_check = {
    "Output": r"/home/sotatek/DucNguyen/ADF_Lottery_Campaign/output/[Lottery Campaign ] Screen Definition_07202026_Ver1.2.xlsx",
    "Ảnh UI (thư mục)": r"/home/sotatek/DucNguyen/ADF_Lottery_Campaign/inputs/screen-detail/image-screen",
    "Screen List": r"/home/sotatek/DucNguyen/ADF_Lottery_Campaign/inputs/screen-detail/screen-list/Screen_List_Ver1.2.xlsx",
    "Function List": r"/home/sotatek/DucNguyen/ADF_Lottery_Campaign/inputs/requirement/business-requirement/Function_List_v1.2.xlsx",
    "Comment Figma": r"/home/sotatek/DucNguyen/ADF_Lottery_Campaign/inputs/screen-detail/spec-note/List_Comment_Figma_Ver1.2.xlsx",
}
...
1a. Tìm ảnh UI cho <TARGET_SCREEN_ID>: đọc ws._images trong sheet hiện tại; nếu không có →
...
2a. Function List — lọc dòng liên quan <TARGET_SCREEN_ID> (cột D/E/F khớp ngữ nghĩa Screen
...
2b. Comment Figma — lọc dòng cho <TARGET_SCREEN_ID>:
    ...
    B2.2: Lọc dòng có Screen ID (đã fill) = "<TARGET_SCREEN_ID>". Nếu không có dòng nào hoặc
          tất cả rỗng D/E/F/G → ghi "Chưa có Comment Figma cho <TARGET_SCREEN_ID>", KHÔNG bịa.
...
═══════════════════════════════════════════════════════════════════════════════
GIAI ĐOẠN III — DỊCH SANG TIẾNG VIỆT cho sheet <SHEET_NAME>
═══════════════════════════════════════════════════════════════════════════════

Sử dụng skill `xlsx`. CHỈ xử lý sheet <SHEET_NAME>, KHÔNG đụng sheet khác, giữ nguyên tên sheet.
...
═══════════════════════════════════════════════════════════════════════════════
L1. GHI ĐÈ TRỰC TIẾP vào chính file output ở B.1 — KHÔNG tạo file mới, không đổi tên. Chỉ sửa
    sheet <SHEET_NAME>; giữ nguyên tất cả sheet khác. Trước khi ghi, đọc lại file đảm bảo không
    mất sheet/định dạng/ảnh; sau khi ghi, xác nhận file mở được và đủ số sheet ban đầu.
L2. SELF-CHECK NGÔN NGỮ: quét sheet <SHEET_NAME>, liệt kê mọi ô còn ký tự tiếng Nhật và xác
...
L3. IN TÓM TẮT CUỐI: (a) kết quả verify GIAI ĐOẠN II (NC), (b) số ô đã dịch GIAI ĐOẠN III,
    (c) danh sách ô còn tiếng Nhật kèm lý do hợp lệ, (d) số ảnh đã chèn, (e) sub-screen đã cover.d